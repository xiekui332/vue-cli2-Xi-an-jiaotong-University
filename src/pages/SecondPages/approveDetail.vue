            ...
            <div class="sh-node-right">
                <div v-if="this.type === 'todo'">
                    ...
                    <div class="sh-node-tips">
                        <div><CommNodeUpload  :isClear='isClear' @handleUpload="handleUpload"/></div>
                        <span>如需上传资料请点击</span> 
                    </div>
...
import CommNodeUpload from '@/components/Common/commNodeUpload'
export default {
    props:[
        'type',
        'proInfo',
        ...
        getProjectStatusMsg(){
            ...
            this.$http.post('/api/project/getSpProjectDetails', params).then(res=>{
                if(res.code=="00000"){
                    ...
                    var lxlzMsg=(dataList[0].list)[1].zlList;
                    var lxMsg=lxzbMsg.concat(lxlzMsg);
                    this.lxzbList=[];
                    for(var i=0;i<lxMsg.length;i++){//立项-资料
                        var obj={};
                        obj.createTime=lxMsg[i].createTime
                        obj.createUserName=lxMsg[i].createUserName;
                        obj.attachName=lxMsg[i].attachName;
                        this.lxzbList.push(obj);
                    }
                    var lxzbqtMsg=(dataList[0].list)[0].qtList;
                    var lxlzqtMsg=(dataList[0].list)[1].qtList;
                    var lxMsg2=lxzbqtMsg.concat(lxlzqtMsg);
                    this.lxzbList2=[];
                    for(var i=0;i<lxMsg2.length;i++){//立项-资料
                        var obj={};
                        obj.createTime=lxMsg2[i].createTime
                        obj.createUserName=lxMsg2[i].createUserName;
                        obj.attachName=lxMsg2[i].attachName;
